...
export const CAMPUS=Object.freeze({name:'Kidsverse School Rehan',latitude:32.1580283,longitude:75.9101877,radiusMeters:5,maxAccuracyMeters:5,mapUrl:'https://maps.app.goo.gl/Rw5mNxZxhuVedfAW8'});
...
export function campusLocation(loc) {
 ...
 insist(Number.isFinite(distance) && loc.accuracy<=CAMPUS.maxAccuracyMeters,`GPS accuracy is Â±${Math.ceil(loc.accuracy)} m. This check-in requires accuracy of 5 m or better. Enable precise location, move to the school map pin, and retry.`,422);
 insist(distance<=CAMPUS.radiusMeters,`You are approximately ${Math.ceil(distance)} m from the school map pin. Attendance is allowed only within 5 m. Move to the approved location and retry.`,403);
 return loc;
}
