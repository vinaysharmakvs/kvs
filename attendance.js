...
export function isSecondSaturday(day) { dateValue(day); const date=new Date(`${day}T12:00:00Z`); return date.getUTCDay()===6 && Number(day.slice(-2))>=8 && Number(day.slice(-2))<=14; }
...
export function campusLocation(loc) {
 ...
 insist(Number.isFinite(distance) && loc.accuracy<=CAMPUS.maxAccuracyMeters,`GPS accuracy is ±${Math.ceil(loc.accuracy)} m. This check-in requires accuracy of 100 m or better. Enable precise location, move to the school map pin, and retry.`,422);
