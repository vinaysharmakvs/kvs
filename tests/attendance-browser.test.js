...
import {hashCode,lookupCode,CAMPUS} from '../lib/attendance-core.js';
test('admin setup and mobile teacher check-in through real API',async()=>{
 ...
 const context=await browser.newContext({viewport:{width:1440,height:1050},permissions:['geolocation'],geolocation:{latitude:CAMPUS.latitude,longitude:CAMPUS.longitude,accuracy:2}});
 ...
 await page.locator('#add-teacher input[name="name"]').fill('Ananya Sharma');await page.getByRole('button',{name:'Create teacher & code'}).click();await page.locator('#new-code code').waitFor();const code=await page.locator('#new-code code').textContent();
 ...
 await page.setViewportSize({width:390,height:844});await page.goto(url+'/teacher-attendance.html');await page.locator('#code').waitFor();await page.screenshot({path:out+'/teacher-login-mobile.png',fullPage:true});
 ...
 const denied=await page.evaluate(async()=>{const r=await fetch('/api/attendance',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'dashboard',session:'teacher'})});return r.status;});assert.equal(denied,403);
 await page.goto(url+'/admin-attendance.html');await page.getByRole('heading',{name:'Teacher attendance',exact:true}).waitFor();
 await page.getByRole('button',{name:'Sign out'}).click();await page.goto(url+'/teacher-attendance.html');await page.getByRole('heading',{name:'Attendance marked',exact:true}).waitFor();
